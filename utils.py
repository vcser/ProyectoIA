...
class TetrisParams:
    ...
    def get_holes(self):
        ...
        for i in range(NUM_COLUMNS):
            ...
            holes_per_col.append(
                peaks[i] - np.sum(self.matrix[-peaks[i]:, i]) if peaks[i] != 0 else 0)
    ...
    def get_wells(self):
        ...
        well_depth[NUM_COLUMNS -
                   1] = max((peaks[NUM_COLUMNS - 2] - peaks[NUM_COLUMNS - 1]), 0)
...
def get_piece_rotations(piece_temp: Piece):
    ...
    for rotation in range(len(piece_temp.shape)):
        ...
        possible_numpy_shapes.append(
            get_numpy_shape(piece_temp.get_formatted_shape()))
        # rotate the piece
    ...
    for rotation, piece in enumerate(piece_rotations):
        ...
        for x_start in range(NUM_COLUMNS - x_len + 1):
            ...
            while y_start + y_len <= NUM_ROWS:
                ...
                matrix[y_start:y_start + y_len,
                       x_start:x_start + x_len] += piece
            ...
            # the calculation is for current_piece
                ...
                if x_start is not None:
                    ...
                    possible_moves_result.append(
                        [rotation, x_start, round(fitness, 5), round(next_piece_fitness, 5)])
